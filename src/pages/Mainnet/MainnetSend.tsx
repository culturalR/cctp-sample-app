...
export const DEFAULT_MAINNET_FORM_INPUTS: TransactionInputs = {
  source: MainnetChain.ETH,
  target: MainnetChain.BASE,
  address: '',
  amount: '',
}

export const MAINNET_CHAIN_SELECT_ITEMS = [
  {
    value: MainnetChain.ETH,
    label: MAINNET_CHAIN_TO_NAME[MainnetChain.ETH],
    icon: CHAIN_ICONS[MainnetChain.ETH],
  },
  {
    value: MainnetChain.BASE,
    label: MAINNET_CHAIN_TO_NAME[MainnetChain.BASE],
    icon: CHAIN_ICONS[MainnetChain.BASE],
  },
]

function MainnetSend() {
  const [formInputs, setFormInputs] = useState<TransactionInputs>(
    DEFAULT_MAINNET_FORM_INPUTS
  )
    ...
    <>
      <div className="item-center mx-auto flex max-w-4xl flex-col justify-center">
        <div className="mb-8 flex items-center justify-center">
          <span className="rounded-md bg-green-100 px-4 py-2 text-green-800">
            Mainnet Mode
          </span>
        </div>
        <h1>Transfer USDC across Mainnet chains</h1>
        ...
        <div className="m-24 flex flex-col">
          <SendForm
            ...
            chainSelectItems={MAINNET_CHAIN_SELECT_ITEMS}
            chainToChainId={MAINNET_CHAIN_TO_ID}
          />
        </div>
      </div>
